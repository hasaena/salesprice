...
function calculatePrice() {
  let result = 0;
  let expectedLanding = ( Number(buyPrice)/11*10.6 + Number(deliveryFee) + Number(weight) * 8.2 ) * 19.19;  
  ...
  if (checkNum(buyPrice) === false || checkNum(deliveryFee) === false || checkNum(weight) === false) {
    ...
    if (expectedLanding > 1500000) {
      ...
      result = Math.ceil(expectedLanding * 1.33 / 10000) * 10000 + 20000;
    } else {
